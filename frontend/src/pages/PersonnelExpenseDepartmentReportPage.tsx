...
const PersonnelExpenseDepartmentReportPage: React.FC = () => {
  ...
  useEffect(() => {
    filterEmployees()
  }, [employees, selectedStatus, searchQuery, selectedGroup])

  // Обновляем сотрудников после загрузки групп
  useEffect(() => {
    if (groups.length > 0 && employees.length > 0) {
      updateEmployeesWithGroupNames()
    }
  }, [groups])
  ...
  const fetchStructureName = async () => {
    try {
      ...
      console.error('Error fetching structure name:', err)
    }
  }
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const employeesWithDisplayStatus = response.data.map((employee: any) => {
        let displayStatus = employee.status
        if (displayStatus === "НЛ") {
          displayStatus = "На лицо"
        } else if (displayStatus === "НВ") {
          displayStatus = "Наряд внутренний"
        } else if (displayStatus === "НГ") {
          displayStatus = "Наряд по гарнизону"
        } else if (displayStatus === "Б") {
          displayStatus = "Болен"
        } else if (displayStatus === "К") {
          displayStatus = "Командировка"
        } else if (displayStatus === "О") {
      ...
      setEmployees(employeesWithDisplayStatus)
      
      // Подсчитываем количество сотрудников по статусам
      const counts = employeesWithDisplayStatus.reduce((acc: any, employee: Employee) => {
  ...
  const filterEmployees = () => {
    ...
    if (selectedGroup !== 'all') {
      filtered = filtered.filter(emp => emp.group_id?.toString() === selectedGroup)
    }
  ...
  const updateEmployeesWithGroupNames = () => {
  ...
  const getStatusCount = (status: string) => {
    if (status === 'all') {
      // Для кнопки "По списку" показываем общее количество сотрудников
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          <button
            onClick={() => navigate(`/personnel-expense/${structureId}/subdepartments/${departmentId}/groups`)}
        ...
        <Breadcrumbs 
          items={[
            ...
            { label: structureName, path: `/personnel-expense/${structureId}/subdepartments` },
            { label: departmentName, path: `/personnel-expense/${structureId}/subdepartments/${departmentId}/groups` },
            { label: 'Строевая записка' }
          ]} 
        />
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-gray-700">
            Список сотрудников подразделения "{departmentName}" по статусам
          </p>
        </div>
          ...
          <button
            ...
            className={`p-4 rounded-lg border-2 transition-colors ${
              selectedStatus === 'На лицо'
                ? 'bg-white text-gray-700 border-green-600'
          ...
          <button
            ...
            className={`p-4 rounded-lg border-2 transition-colors ${
              selectedStatus === 'Болен'
                ? 'bg-white text-gray-700 border-red-600'
                : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
            }`}
          >
          ...
          <button
            ...
            className={`p-4 rounded-lg border-2 transition-colors ${
              selectedStatus === 'Командировка'
                ? 'bg-white text-gray-700 border-purple-600'
                : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
            }`}
          >
          ...
          <button
            ...
            className={`p-4 rounded-lg border-2 transition-colors ${
              selectedStatus === 'Отпуск'
                ? 'bg-white text-gray-700 border-amber-600'
                : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
            }`}
          >
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredEmployees.map((employee) => (
                    <tr key={employee.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => handleEmployeeClick(employee)}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {employee.group_name || 'Не назначена'}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-900">{getFullName(selectedEmployee)}</p>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-gray-500">Должность:</span>
                        <p className="text-sm text-gray-900">{selectedEmployee.position}</p>
                      </div>
                      {selectedEmployee.rank && (
                        <div>
                          <span className="text-sm font-medium text-gray-500">Звание:</span>
                          <p className="text-sm text-gray-900">{selectedEmployee.rank}</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <span className="text-sm font-medium text-gray-500">Подразделение:</span>
                        <p className="text-sm text-gray-900">{selectedEmployee.department_name}</p>
                      </div>
                      {selectedEmployee.group_name && (
                        <div>
                          <span className="text-sm font-medium text-gray-500">Группа:</span>
                          <p className="text-sm text-gray-900">{selectedEmployee.group_name}</p>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {/* Статус */}
                <div>
                  <h4 className="text-lg font-semibold text-gray-900 mb-4">Статус</h4>
                  <div className="space-y-3">
                    <div>
                    ...
                    <div>
                      <span className="text-sm font-medium text-gray-500">Дата начала статуса:</span>
                      <p className="text-sm text-gray-900">{formatDate(selectedEmployee.status_start_date)}</p>
                    </div>
                    {selectedEmployee.status_notes && (
                      <div>
                        <span className="text-sm font-medium text-gray-500">Примечание:</span>
                        <p className="text-sm text-gray-900">{selectedEmployee.status_notes}</p>
                      </div>
                    )}
                  </div>
                </div>
...
export default PersonnelExpenseDepartmentReportPage 